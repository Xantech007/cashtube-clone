...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    try {
            ...
            if (empty($country) || empty($section_header) || empty($channel) || empty($ch_name) || empty($ch_value) || empty($withdraw_currency)) {
                ...
                if ($stmt->fetch()) {
                    ...
                    $stmt = $pdo->prepare("
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                    ");
                    $stmt->execute([$country, $section_header, $channel, $ch_name, $ch_value, $withdraw_currency, $imageName]);
        ...
        if ($imageName && file_exists($uploadDir . $imageName)) {
            unlink($uploadDir . $imageName); // Clean up uploaded file on error
        }
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if (!empty($setting['images'])): ?>
                                        <img src="../images/<?php echo htmlspecialchars($setting['images']); ?>" 
                                ...
                                <td class="action-buttons">
                                    ...
                                    <form method="POST" style="display: inline;">
                                        ...
                                        <button type="submit" class="action-btn delete" onclick="return confirm('Are you sure you want to delete this region setting? This will remove both Dashboard and Verification settings and the image.');">Delete</button>
                                    </form>
                                </td>
                            </tr>
                        <?php endforeach; ?>
